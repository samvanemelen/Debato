      ...
      */
      ...
      Promise.all(promiseList).then((argumentParents) => {
        ...
        for (let i = 0; i < argumentParents.length; i += 1) {
          ...
          body += `<p style="font-size: 0.9em; color: ">Argumented (${postType}) on: <a class="parentLink" href="/html/discussion?a=${argumentParents[i].author}&p=${argumentParents[i].perm}">${argumentParents[i].title}</a></p>`;
